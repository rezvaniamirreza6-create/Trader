...
SYMBOL_DISPLAY_NAMES = {
    ...
    "DJI": "شاخص داوجونز (DJI)",
    "SPX": "شاخص اس‌اندپی ۵۰۰ (SPX)",
    "IXIC": "شاخص نزدک (IXIC)",
}

# نمادهایی که فقط در ساعات کاری بورس آمریکا داده دارند (نه ۲۴/۷)
STOCK_INDEX_SYMBOLS = {"DJI", "SPX", "IXIC"}
...
def main_keyboard():
    ...
    buttons = [
        ...
        [KeyboardButton("📰 اخبار اقتصادی"), KeyboardButton("ℹ️ راهنما")],
    ]
...
def market_selection_inline_keyboard():
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("📈 داوجونز", callback_data="selectsym_DJI"),
            InlineKeyboardButton("📊 اس‌اندپی۵۰۰", callback_data="selectsym_SPX"),
            InlineKeyboardButton("💻 نزدک", callback_data="selectsym_IXIC"),
        ],
    ]
...
async def news_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    نمایش اخبار اقتصادی مهم پیش رو و اخیراً منتشرشده.
    این تابع نتیجه‌ی بازار را پیش‌بینی نمی‌کند؛ فقط می‌گوید چه خبری
    در راه است (ریسک نوسان) و عدد واقعی چطور با پیش‌بینی فرق داشت (فکت).
    """
    if not FINNHUB_API_KEY:
        await update.message.reply_text(
            "❌ کلید API اخبار تنظیم نشده است.\n"
            "ادمین باید متغیر FINNHUB_API_KEY را در Railway تنظیم کند.\n"
            "کلید رایگان: https://finnhub.io/register"
        )
        return
...
async def handle_symbol_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    symbol = query.data.split("_", 1)[1]  # مثلا: BTC/USD یا DJI
...
async def handle_analysis_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # هشدار ریسک خبری (در صورت موجود بودن کلید Finnhub) - اگر خطا داد، تحلیل تکنیکال ادامه می‌یابد
        news_warning = None
        if FINNHUB_API_KEY:
            news_warning = nc.get_news_risk_warning(FINNHUB_API_KEY, within_hours=6)
        ...
        if symbol in STOCK_INDEX_SYMBOLS:
            text += (
                "ℹ️ این شاخص فقط در ساعات کاری بورس آمریکا معامله می‌شود؛ "
                "خارج از این ساعات ممکن است داده بازار بسته یا کم‌حجم باشد.\n\n"
            )
