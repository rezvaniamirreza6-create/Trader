...
async def handle_analysis_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # outputsize=400 (نه 200) چون تایید چندتایم‌فریمی به کندل‌های
        # بیشتری نیاز دارد تا بتواند تایم‌فریم ۴ برابر بزرگ‌تر را بسازد
        df = analysis.get_market_data(symbol=symbol, api_key=TWELVE_DATA_API_KEY, interval=interval, outputsize=400)
        result = analysis.generate_mtf_confirmed_signal(df, higher_tf_factor=4)
