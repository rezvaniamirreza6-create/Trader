...
# ضریب تایم‌فریم بالاتر برای تایید چندتایم‌فریمی - برای تایم‌فریم‌های
# خیلی کوتاه (۱ و ۵ دقیقه)، ضریب ۴ معادل بزرگ‌تری لازم دارد تا تایم‌فریم
# "بالاتر" واقعاً معنادار باشد (نه فقط چند دقیقه دیگر).
# مثال: ۱ دقیقه × factor=15 = ۱۵ دقیقه (معنادار)؛ ۱ ساعت × factor=4 = ۴ ساعت (معنادار)
MTF_FACTOR_BY_TIMEFRAME = {
    "1min": 15,   # 1min -> معادل 15min
    "5min": 12,   # 5min -> معادل 1h
    "15min": 4,   # 15min -> معادل 1h
    "30min": 4,   # 30min -> معادل 2h
    "1h": 4,      # 1h -> معادل 4h
    "4h": 6,      # 4h -> معادل 1day
    "1day": 7,    # 1day -> معادل 1week (تقریبی)
}
...
def timeframe_selection_inline_keyboard(symbol):
    ...
    keyboard = [
        [
            InlineKeyboardButton("۱ دقیقه", callback_data=f"analyze_{symbol}_1min"),
            InlineKeyboardButton("۵ دقیقه", callback_data=f"analyze_{symbol}_5min"),
        ],
        [
...
def admin_panel_inline_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📐 بک‌تست Walk-Forward (تک‌بازار)", callback_data="admin_backtest")],
        [InlineKeyboardButton("🌍 بک‌تست چند-بازاری (پایداری)", callback_data="admin_multibacktest")],
...
async def handle_analysis_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # برای تایم‌فریم‌های خیلی کوتاه (1min, 5min)، ضریب تایم‌فریم بالاتر
        # بزرگ‌تر است (طبق MTF_FACTOR_BY_TIMEFRAME)، پس به کندل بیشتری
        # نیاز داریم تا حداقل ۶۵ گروه برای محاسبه اندیکاتور آن فراهم شود.
        mtf_factor = MTF_FACTOR_BY_TIMEFRAME.get(interval, 4)
        required_outputsize = max(400, mtf_factor * 65)
        df = analysis.get_market_data(symbol=symbol, api_key=TWELVE_DATA_API_KEY, interval=interval, outputsize=required_outputsize)
        result = analysis.generate_mtf_confirmed_signal(df, higher_tf_factor=mtf_factor)
        ...
        # نمایش شفاف وضعیت تایید چندتایم‌فریمی (Multi-Timeframe Confluence)
        if result.get("mtf_confirmed") is True:
            text += f"\n✅ تایم‌فریم بالاتر (۴ برابر) تایید می‌کند: {result['higher_tf_verdict']}"
        elif result.get("mtf_confirmed") is False:
            text += f"\n⚠️ تایم‌فریم بالاتر تایید نمی‌کند ({result['higher_tf_verdict']}) — سیگنال رد شد، خنثی در نظر گرفته می‌شود."
        elif result.get("higher_tf_verdict"):
            text += f"\nℹ️ {result['higher_tf_verdict']}"
...
async def handle_admin_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif action == "admin_multibacktest":
        await run_multi_symbol_backtest_for_admin(query)
...
# نمادهایی که در بک‌تست چند-بازاری به‌صورت ثابت بررسی می‌شوند
MULTI_BACKTEST_SYMBOLS = ["BTC/USD", "EUR/USD", "XAU/USD", "GBP/USD"]


async def run_multi_symbol_backtest_for_admin(query):
    """
    اجرای بک‌تست Walk-Forward روی چند بازار مختلف به‌طور همزمان و
    مقایسه‌ی پایداری عملکرد بین آن‌ها. این به سوال مهم‌تری از «دقت
    روی یک بازار خاص چقدر است» پاسخ می‌دهد: «آیا این موتور به‌طور
    کلی و در بازارهای مختلف پایدار است، یا فقط شانسی روی یک بازار
    خاص خوب به نظر می‌رسد؟»

    ⚠️ این عملیات به ازای هر بازار، یک درخواست جدا به Twelve Data
    می‌زند (مثلاً ۴ درخواست برای ۴ بازار) - نه یک درخواست واحد مثل
    بک‌تست تک‌بازاری.
    """
    user_id = query.from_user.id
    if not is_admin(user_id):
    ...
    await query.answer()
    await query.edit_message_text(
        f"⏳ در حال اجرای بک‌تست روی {len(MULTI_BACKTEST_SYMBOLS)} بازار مختلف "
        f"({', '.join(MULTI_BACKTEST_SYMBOLS)}) ...\n"
        f"این عملیات ممکن است ۱ تا ۲ دقیقه طول بکشد و {len(MULTI_BACKTEST_SYMBOLS)} "
        f"درخواست به Twelve Data می‌زند."
    )
    ...
    try:
        symbol_dataframes = {}
        for symbol in MULTI_BACKTEST_SYMBOLS:
            try:
                df = analysis.get_market_data(symbol=symbol, api_key=TWELVE_DATA_API_KEY, interval="1h", outputsize=1000)
                symbol_dataframes[symbol] = df
            except Exception as e:
                logger.warning(f"دریافت داده برای بک‌تست چند-بازاری ({symbol}) ناموفق: {e}")

        if not symbol_dataframes:
            await query.edit_message_text("❌ دریافت داده برای هیچ‌کدام از بازارها موفق نبود.")
            return

        result = bte.run_multi_symbol_backtest(symbol_dataframes, n_windows=4, horizon=5)
        report = bte.format_multi_symbol_report(result)
        await query.edit_message_text(report, parse_mode="Markdown")
    except Exception as e:
        logger.error(f"خطا در اجرای بک‌تست چند-بازاری: {e}")
        await query.edit_message_text(f"❌ خطا در اجرای بک‌تست:\n{str(e)}")
