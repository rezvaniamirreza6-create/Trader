...
def run_walk_forward_backtest(df, n_windows=6, horizon=5, min_window_data=65,
                                user_weight_multipliers=None, use_mtf=False, mtf_factor=4):
    """
    اجرای بک‌تست Walk-Forward روی یک دیتافریم OHLC واقعی یا شبیه‌سازی‌شده.
    ...
    min_window_data: حداقل کندل لازم در ابتدای هر پنجره برای محاسبه اندیکاتورها
    user_weight_multipliers: دیکشنری اختیاری وزن شخصی کاربر (برای بک‌تست استراتژی شخصی)
    use_mtf: اگر True باشد، به‌جای generate_signal از generate_mtf_confirmed_signal
             استفاده می‌شود (تایید چندتایم‌فریمی) - برای مقایسه‌ی دو روش
    mtf_factor: ضریب تایم‌فریم بالاتر برای تایید چندتایم‌فریمی

    خروجی: دیکشنری شامل نتیجه هر پنجره + خلاصه‌ی آماری کلی
    """
    n_total = len(df)
    min_required = n_windows * min_window_data * 2
    if use_mtf:
        min_required = max(min_required, min_window_data * mtf_factor * 2)
    if n_total < min_required:
        return {
            "error": (
                f"داده کافی برای {n_windows} پنجره وجود ندارد. "
                f"حداقل {min_required} کندل لازم است، "
                f"اما فقط {n_total} کندل موجود است."
            )
        }
    ...
    for w in range(n_windows):
        ...
        for i in range(start_idx + min_window_data, min(end_idx, n_total - horizon)):
            ...
            try:
                if use_mtf:
                    result = analysis.generate_mtf_confirmed_signal(window_df, higher_tf_factor=mtf_factor)
                else:
                    result = analysis.generate_signal(window_df, user_weight_multipliers=user_weight_multipliers)
            except Exception:
                continue
            if "error" in result or result["score"] == 0:
                if use_mtf and result.get("mtf_confirmed") is False:
                    rejected_count += 1
                continue
    ...
    if use_mtf:
        result_dict["rejected_by_mtf"] = rejected_count

    return result_dict


def run_multi_symbol_backtest(symbol_dataframes, n_windows=4, horizon=5, use_mtf=False):
    """
    اجرای بک‌تست Walk-Forward روی چند نماد مختلف و میانگین‌گیری نتیجه.

    این تابع برای پاسخ به یک سوال مهم طراحی شده: «آیا موتور تحلیل فقط
    روی یک بازار خاص خوب عمل می‌کند، یا عملکردش در بازارهای مختلف
    پایدار است؟» اگر دقت بین نمادهای مختلف خیلی متفاوت باشد، یعنی
    هیچ مزیت ساختاریافته‌ای وجود ندارد - فقط شانس در یک بازار خاص بوده.

    symbol_dataframes: دیکشنری {symbol_name: dataframe}
    خروجی: نتیجه به تفکیک هر نماد + خلاصه‌ی کلی بین‌بازاری
    """
    per_symbol_results = {}

    for symbol, df in symbol_dataframes.items():
        result = run_walk_forward_backtest(df, n_windows=n_windows, horizon=horizon, use_mtf=use_mtf)
        per_symbol_results[symbol] = result

    valid_results = {s: r for s, r in per_symbol_results.items() if "error" not in r}

    if not valid_results:
        return {"error": "هیچ نمادی داده کافی برای بک‌تست نداشت.", "per_symbol": per_symbol_results}

    mean_accuracies = [r["mean_accuracy"] for r in valid_results.values()]

    return {
        "per_symbol": per_symbol_results,
        "cross_symbol_mean": round(float(np.mean(mean_accuracies)), 1),
        "cross_symbol_std": round(float(np.std(mean_accuracies)), 1),
        "best_symbol": max(valid_results.items(), key=lambda x: x[1]["mean_accuracy"])[0],
        "worst_symbol": min(valid_results.items(), key=lambda x: x[1]["mean_accuracy"])[0],
    }


def format_multi_symbol_report(result):
    """تولید گزارش متنی فارسی از نتیجه بک‌تست چند-نمادی."""
    if "error" in result:
        return f"❌ {result['error']}"

    lines = ["📊 *گزارش بک‌تست چند-بازاری (Walk-Forward)*\n"]

    for symbol, sym_result in result["per_symbol"].items():
        if "error" in sym_result:
            lines.append(f"🔹 {symbol}: ❌ {sym_result['error']}")
            continue
        lines.append(
            f"🔹 {symbol}: دقت {sym_result['mean_accuracy']}% "
            f"({sym_result['total_signals']} سیگنال, نوسان {sym_result['std_accuracy']}%)"
        )

    lines.append("")
    lines.append(f"📈 میانگین بین‌بازاری: {result['cross_symbol_mean']}%")
    lines.append(f"📉 انحراف بین بازارها: {result['cross_symbol_std']}% (هرچه کمتر، پایدارتر بین بازارها)")
    lines.append(f"🔼 بهترین بازار: {result['best_symbol']} | 🔽 بدترین بازار: {result['worst_symbol']}")

    lines.append(
        "\nℹ️ اگر انحراف بین بازارها بالا باشد، یعنی موتور تحلیل مزیت "
        "ساختاریافته و پایداری ندارد و عملکرد خوب در یک بازار خاص می‌تواند "
        "صرفاً شانس بوده باشد، نه یک الگوی واقعی قابل‌تکرار."
    )
...
def format_backtest_report(result, symbol="نامشخص"):
    ...
    if "rejected_by_mtf" in result:
        total_checked = result["total_signals"] + result["rejected_by_mtf"]
        rejection_pct = round(result["rejected_by_mtf"] / total_checked * 100, 1) if total_checked else 0
        lines.append(f"🚫 سیگنال‌های رد‌شده توسط تایید چندتایم‌فریمی: {result['rejected_by_mtf']} ({rejection_pct}%)")
